...
docker pull --quiet  docker.pkg.github.com/ninja-software/boilerplate/boilerplate:latest &> /dev/null

echo "Removing existing containers..."

[ "$(docker ps -a | grep boilerplate)" ] && docker stop boilerplate &> /dev/null
[ "$(docker ps -a | grep boilerplate)" ] && docker rm boilerplate &> /dev/null

echo "Migrating database..."

docker run \
        --rm \
        --name boilerplate-db-client \
...
docker.pkg.github.com/ninja-software/boilerplate/boilerplate:latest db-migrate
...
docker run \
        ...
        -e BOILERPLATE_DATABASE_PORT=$BOILERPLATE_DATABASE_PORT \
        -e BOILERPLATE_DATABASE_NAME=$BOILERPLATE_DATABASE_NAME \
docker.pkg.github.com/ninja-software/boilerplate/boilerplate:latest
